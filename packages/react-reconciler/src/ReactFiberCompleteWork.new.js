...
function hadNoMutationsEffects(current: null | Fiber, completedWork: Fiber) {
  ...
  while (child !== null) {
    if ((child.flags & (MutationMask | ChildDeletion)) !== NoFlags) {
      return false;
    }
    if ((child.subtreeFlags & (MutationMask | ChildDeletion)) !== NoFlags) {
      return false;
    }
    child = child.sibling;
  }
  ...
  switch (workInProgress.tag) {
    ...
    case Profiler: {
      ...
      if (!didBailout) {
        ...
        if (
          (flags & (LayoutMask | ChildDeletion)) !== NoFlags ||
          (subtreeFlags & (LayoutMask | ChildDeletion)) !== NoFlags
        ) {
          newFlags |= OnCommitFlag;
        }

        // Call onPostCommit only if the subtree contains passive work.
        // Don't have to check for deletions, because ChildDeletion is already
        // a passive flag.
        if (
